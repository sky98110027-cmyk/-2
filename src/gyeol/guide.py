...
5. 영상 만들기
   영상 한 편은 타임라인 하나 위에 레이어 여섯이 얹힌 것이다.
   화면, 캐릭터, 자막, 목소리, 환경음, 배경음.
   레이어마다 제공자를 꽂는다. 목소리는 일레븐랩스, 화면은 힉스필드나 내 그림 폴더.
   레퍼런스를 프레임까지 뜯어두면 새 영상이 그 모양을 따라간다.
   장면 수, 침묵 자리, 캐릭터 뜨는 자리까지 비례로 옮긴다.
   재료가 다 모이면 ffmpeg 로 묶어서 mp4 를 만든다.
...
- 영상을 통째로 보지는 못한다. ffmpeg 가 있으면 장면이 바뀌는 지점마다 한 장씩 뽑아서 본다.
  그 한 장으로 구도, 색감, 화면 위 글씨, 밈 캐릭터가 어디 뜨는지를 읽는다.
  ffmpeg 가 없으면 자막과 제목과 설명만 읽는다.
- 환경음이 무슨 소리인지는 못 듣는다. 말이 없는 구간이 침묵인지 음악만 흐르는지는 잰다.
- 장면 그림을 직접 그리지 못한다. 장면별 영문 프롬프트를 만들고,
  실제 그림은 힉스필드나 톱뷰 같은 도구가 뽑는다. 그림이 준비되면 ffmpeg 로 묶어 mp4 를 만든다.
